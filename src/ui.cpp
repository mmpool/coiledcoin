...
bool GetWalletPassphrase()
{
    if (pwalletMain->IsLocked())
    {
        ...
        if (!strWalletPass.size())
        {
            ...
            wxMessageBox(_("Please supply the current wallet decryption passphrase."), "CedarCoin");
            return false;
        }
        ...
        if (!pwalletMain->Unlock(strWalletPass))
        {
            fill(strWalletPass.begin(), strWalletPass.end(), '\0');
            munlock(&strWalletPass[0], strWalletPass.capacity());
            wxMessageBox(_("The passphrase entered for the wallet decryption was incorrect."), "CedarCoin");
            return false;
        }
        fill(strWalletPass.begin(), strWalletPass.end(), '\0');
        munlock(&strWalletPass[0], strWalletPass.capacity());
...
void CMainFrame::OnMenuOptionsEncryptWallet(wxCommandEvent& event)
{
    ...
    if (pwalletMain->IsCrypted())
    {
        wxMessageBox(_("Wallet already encrypted."), "CedarCoin", wxOK | wxICON_ERROR);
        return;
    }
    ...
    if (!strWalletPass.size())
    {
        fill(strWalletPass.begin(), strWalletPass.end(), '\0');
        munlock(&strWalletPass[0], strWalletPass.capacity());
        wxMessageBox(_("Error: The supplied passphrase was too short."), "CedarCoin", wxOK | wxICON_ERROR);
        return;
    }

    if(wxMessageBox(_("WARNING: If you encrypt your wallet and lose your passphrase, you will LOSE ALL OF YOUR CEDARCOINS!\nAre you sure you wish to encrypt your wallet?"), "CedarCoin", wxYES_NO) != wxYES)
        return;
    ...
    if (strWalletPassTest != strWalletPass)
    {
        ...
        wxMessageBox(_("Error: the supplied passphrases didn't match."), "CedarCoin", wxOK | wxICON_ERROR);
        return;
    }
    ...
    if (!pwalletMain->EncryptWallet(strWalletPass))
    {
        ...
        munlock(&strWalletPass[0], strWalletPass.capacity());
        munlock(&strWalletPassTest[0], strWalletPassTest.capacity());
        wxMessageBox(_("Wallet encryption failed."), "CedarCoin", wxOK | wxICON_ERROR);
        return;
    }
    fill(strWalletPass.begin(), strWalletPass.end(), '\0');
    fill(strWalletPassTest.begin(), strWalletPassTest.end(), '\0');
    munlock(&strWalletPass[0], strWalletPass.capacity());
    munlock(&strWalletPassTest[0], strWalletPassTest.capacity());
    wxMessageBox(_("Wallet Encrypted.\nRemember that encrypting your wallet cannot fully protect your cedarcoins from being stolen by malware infecting your computer."), "CedarCoin");
...
void CMainFrame::OnMenuOptionsChangeWalletPassphrase(wxCommandEvent& event)
{
    ...
    if (!pwalletMain->IsCrypted())
    {
        wxMessageBox(_("Wallet is unencrypted, please encrypt it first."), "CedarCoin", wxOK | wxICON_ERROR);
        return;
    }
    ...
    if (!strOldWalletPass.size() || !pwalletMain->Unlock(strOldWalletPass))
    {
        ...
        wxMessageBox(_("The passphrase entered for the wallet decryption was incorrect."), "CedarCoin", wxOK | wxICON_ERROR);
        return;
    }
    ...
    munlock(&strNewWalletPass[0], strNewWalletPass.capacity());
    munlock(&strNewWalletPassTest[0], strNewWalletPassTest.capacity());
    wxMessageBox(_("Wallet Passphrase Changed."), "CedarCoin");
}
...
{
    return MyGetSpecialFolderPath(CSIDL_STARTUP, true) + "\\CedarCoin.lnk";
}
...
{
    return GetAutostartDir() / boost::filesystem::path("cedarcoin.desktop");
}
...
void SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
        {
            wxMessageBox(_("Cannot write autostart/cedarcoin.desktop file"), "CedarCoin");
            return;
        }
        // Write a bitcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=CedarCoin\n";
...
#endif
#if defined(__WXGTK__) || defined(__WXMAC_OSX__)
    m_checkBoxStartOnSystemStartup->SetLabel(_("&Start CedarCoin on window system startup"));
    if (!GetBoolArg("-minimizetotray"))
    {
...
CAddressBookDialog::CAddressBookDialog(wxWindow* parent, const wxString& strInitSelected, int nPageIn, bool fDuringSendIn) : CAddressBookDialogBase(parent)
{
    ...
    m_listCtrlReceiving->InsertColumn(1, _("CedarCoin Address"), wxLIST_FORMAT_LEFT, 350);
...
void CMyTaskBarIcon::Show(bool fShow)
{
    ...
    if (fShow)
    {
        string strTooltip = _("CedarCoin");
        if (fGenerateBitcoins)
            strTooltip = _("CedarCoin - Generating");
        if (fGenerateBitcoins && vNodes.empty())
            strTooltip = _("CedarCoin - (not connected)");
...
wxMenu* CMyTaskBarIcon::CreatePopupMenu()
{
    ...
    pmenu->Append(ID_TASKBAR_RESTORE, _("&Open CedarCoin"));
    pmenu->Append(ID_TASKBAR_SEND, _("&Send CedarCoins"));
...
{
    wxMessageBox(_("Program has crashed and will terminate.  "), "CedarCoin", wxOK | wxICON_ERROR);
}
